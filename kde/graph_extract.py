...
def pixels_to_coords(xxx_todo_changeme1):
    i, j = xxx_todo_changeme1
    return (((height - i) / yscale) + min_lat), ((j / xscale) + min_lon)

class Node:
    def __init__(self, xxx_todo_changeme, weight):
...
class Graph:
    ...
    def create_nodes_and_new_segments(self, main_crossings, segments, density_estimate):
        density_map = [2**x for x in range(16, 3, -1)] + list(range(15, 0, -1))
        density_map.reverse()
    ...
    def find_main_crossings_and_segments(self, skeleton):
        ...
        for i, j in zip(crossing_pixels[0], crossing_pixels[1]):
    ...
    def identify_crossing_points(self, skeleton):
        ...
        for i, j in zip(fg_pixels[0], fg_pixels[1]):
            if (curr_count % 100 == 0) or (curr_count == total_count):
                print("\r" + str(curr_count) + "/" + str(total_count) + "... ")
            curr_count += 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Extract map graph from skeleton image')
    parser.add_argument('--skeleton_image_path', type=str, help='Path to skeleton image file')
    parser.add_argument('--bounding_box_path', type=str, help='Path to area bounding box')
    parser.add_argument('--output_file_path', type=str, help='Path to save output sqlite database file')
