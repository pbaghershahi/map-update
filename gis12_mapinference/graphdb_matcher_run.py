...
# ambiguity: what is the difference between this threshold and the one in graphdb_matcher
MAX_DIST_SUBDIVISION_VITERBI = 10.0
...
class MatchGraphDB:
    ...
    def process_trip(self, trips, csv_writer):
        ...
        for i in range(0, len(obs)):
            ...
            # csv_writer.writerow(obs[i])
            # print(','.join([str(x) for x in obs[i]]))
            # text_towrite = str(obs_lat) + " " + str(obs_lon) + " " + str(obs_time) + " "
            row = list(obs[i])
            if obs_states[i] == "unknown":
                # out_file.write(str(obs_states[i]) + "\n")
                row += ['unknown']
                # text_towrite += str(obs_states[i]) + "\n"
            else:
                ...
                # print(text_towrite+str(in_node_coords[0]) + " " + str(in_node_coords[1]) + " " + str(out_node_coords[0]) + " " + str(out_node_coords[1]) + "\n")
                # text_towrite += str(in_node_coords[0]) + " " + str(in_node_coords[1]) + " " + str(out_node_coords[0]) + " " + str(out_node_coords[1]) + "\n"
...
if __name__ == '__main__':
    ...
    for o,a in opts:
        if o == "-c":
            ...
            #note: maximum distance to detect nearby areas in Rtree algorithm (areas are representations of map edges)
            max_dist = int(a)
        elif o == "-d":
            ...
            print("Usage: python graphdb_matcher_run.py \
            [-c <constraint_length>] \
            [-m <max_dist>] \
            [-d <graphdb_filename>] [-t <trip_directory>] [-o <output_directory>] [-h]")
            exit()
    ...
    for dir_name in [x for x in os.listdir(trip_directory) if os.path.isdir(os.path.join(trip_directory, x)) and not x.startswith('.')]:
        ...
        for file_name in file_names:
            ...
            # modified_trips = TripLoader.load_trip_from_file(os.path.join(trip_directory, dir_name, file_name))
