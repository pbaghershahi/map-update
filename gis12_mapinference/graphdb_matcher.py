...
class GraphDBMatcher(GPSMatcher):
    def __init__(self, mapdb, constraint_length=300, MAX_DIST=100):
        ...
        emission_probabilities = [complementary_normal_distribution_cdf(x,0,EMISSION_SIGMA) for x in range(0,int(3.0*EMISSION_SIGMA))]
    ...
    def mapdb_to_hmm(self,mapdb):
        ...
        hmm['unknown'] += [((edge.in_node.coords(), edge.out_node.coords()), (1 - TRANSITION_UNKNOWN_UNKNOWN) / len(themap.edges)) for edge in list(themap.edges.values())]
