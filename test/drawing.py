...
if __name__ == '__main__':
    ...
    Rc = 0.0009
    r = 0.0008
    V_set, adj_matrix, dists, total_nodes = gen_cobweb(traj_file, Rv, Rc, unmatch, match_file)
    G = nx.from_numpy_array(adj_matrix)
    fixed_positions = dict(zip(np.arange(0, V_set.shape[0], 1), V_set))
    pos = nx.spring_layout(G, pos=fixed_positions, fixed=G.nodes())
    options = {"node_size": 50, "font_size": 10, "alpha": 0.8, "with_labels": True, "edge_color": 'c', "font_color": 'k'}
