...
unmatched_indices = matches[matches.unmatch==False].index
...
#
# filtered_edges['num_trips'] = 0
# for match_idx, row in trajs_matches.iterrows():
#     for edge_idx in [int(k) for k in row.cpath.split(',')]:
#         filtered_edges.num_trips.loc[edge_idx] += 1
# dropped_edges = filtered_edges[filtered_edges.num_trips > 0]
#
# filtered_edges.plot()
