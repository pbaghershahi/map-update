...
trips_path   = [pwd,'\data\berlin\trips\'];
output_path = [pwd,'\results\berlin\'];
...
% tic;
trip = tripRead(trips_path, 'ahmed');
% interp_dist = 50; 
...
option.spatial_threshold  = 10;     % 相邻轨迹点之间的横向距离（垂直于运动方向）阈值 m
...
stretch_dist = 30; % 考虑到由于GPS定位存在误差，有些垂直的悬挂线在延伸的情况不相交，为此，对各悬挂线首先进行首尾的拉伸，该参数即为各路段首尾延伸的距离（以GPS定位误差进行设置）。
