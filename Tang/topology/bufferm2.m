...
%bufferm  Computes buffer zone around a polygon.
%
% Usage:
...
%   [xb,  yb]   = bufferm('xy',x,y,dist,direction,npts,outputformat)
%
...
%   [latb, lonb] = csu_bufferm(reducedlat, reducedlon, dist, 'out');
...
if ischar(varargin{1}) && strcmp(varargin{1}, 'xy')
    ...
    p = isnan(varargin{2}(end));
    is_polygon = (varargin{2}(1)==varargin{2}(end-p)) && (varargin{3}(1)==varargin{3}(end-p));
else
    geo = true;
    param = varargin;
    
    p = isnan(varargin{1}(end));
    is_polygon = (varargin{1}(1)==varargin{1}(end-p)) && (varargin{2}(1)==varargin{2}(end-p));
end
...
if ~is_polygon
    if length(loncrall)>1
        [lonb, latb] = polybool('+', loncrall(1), latcrall(1), loncrall(2:end), latcrall(2:end));
    else
        lonb = loncrall;
        latb = latcrall;
    end
